...
def ba(func, dim=100, n=200, maxiter=2000, is_min=True, topk=100, topk_coef=10, fmin=0, fmax=1, loud_A=0.95, r0=0.85, alpha=0.999, gamma=0.1,
       w_init=0.9, n_batch=1,
       w_final=0.2, seed=None, progress=False, device=None, low_mem=False):
    """
    ...
    for t in m:
        ...
        for i in range(n_batch):
            v[i * n_per_batch:(i + 1) * n_per_batch] += (x[i * n_per_batch:(i + 1) * n_per_batch] - best_s) * freq[i * n_per_batch:(
                                                                                                                                           i + 1) * n_per_batch]
            ...
            x_new = torch.where(accept, best_s.expand(n_per_batch, dim) + loud_A * torch.randn(n_per_batch, dim, device=device, dtype=dtype),
                                x_new)
            ...
            x[i * n_per_batch:(i + 1) * n_per_batch] = torch.where(accept.expand(n_per_batch, dim), x_new,
                                                                   x[i * n_per_batch:(i + 1) * n_per_batch])
    ...
    best_s_list = torch.unique(torch.cat((fit[idx], x[idx]), dim=1), dim=0)[0:topk, 1:]
